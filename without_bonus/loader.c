 ...
 */
void add_node(void *addr,int size)
{
...
/*
 * Finds the segment and alloacte memory in multiple of PAGE_SIZE
 */
...
void segmentation_handler(int signum, siginfo_t *info, void *no_use)
{
    ...
    for (int i = 0; i < number_of_program; i++)
    {
        ...
        if (((phdr->p_vaddr <= (int)(info->si_addr)) && ((int)info->si_addr < phdr->p_vaddr + phdr->p_memsz)))
        {
            break;
        }
    }

    int num_page = math_ceil(PAGE_SIZE, phdr->p_memsz);
    num_page_allocations += num_page;
    internal_fragmentation += num_page * PAGE_SIZE - phdr->p_memsz;
    void *virtual_mem = mmap((void *)phdr->p_vaddr, PAGE_SIZE * num_page, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0);
    if(virtual_mem==MAP_FAILED){
        printf("Failed to allocate memory\n");
    }
    add_node(virtual_mem,PAGE_SIZE * num_page);
    memcpy(virtual_mem, (void *)ehdr + phdr->p_offset, phdr->p_filesz);
}
